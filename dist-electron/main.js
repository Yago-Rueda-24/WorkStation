...
class SysInfoEntity {
  ...
  static fromPlain(data) {
    ...
    if (data.cpuLoad !== void 0) entity.setCpuLoad(data.cpuLoad);
    if (data.cpus !== void 0) entity.setCpus(data.cpus);
    if (data.memoryLoad !== void 0) entity.setMemoryLoad(data.memoryLoad);
    if (data.networkLoad !== void 0) entity.setNetworkLoad(data.networkLoad);
    if (data.diskLoad !== void 0) entity.setDiskLoad(data.diskLoad);
    if (data.gpuLoad !== void 0) entity.setGpuLoad(data.gpuLoad);
    return entity;
  }
}
class SysInfoService {
  sysInfoPort;
  constructor(sysInfoPort) {
  ...
  async getAll() {
    const [cpu2, mem, net, disk, gpu] = await Promise.all([
...
function requireSystem() {
  ...
  const exec = require$$1.exec;
  const execSync = require$$1.execSync;
  const execPromise = util2.promisify(require$$1.exec);
  const _platform = process.platform;
  const _linux = _platform === "linux" || _platform === "android";
  const _darwin = _platform === "darwin";
  const _windows = _platform === "win32";
            ...
            if (result2.manufacturer === "" && result2.model === "Computer" && result2.version === "") {
              fs.readFile("/proc/cpuinfo", (error2, stdout2) => {
                if (!error2) {
                  ...
                  util2.getValue(lines2, "model:", ":", true);
...
function requireBattery() {
  ...
  battery = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
      let result2 = {
        hasBattery: false,
        cycleCount: 0,
...
function requireGraphics() {
  ...
  function graphics$1(callback) {
    ...
    function getNvidiaSmi() {
      ...
      if (_windows) {
        try {
          const basePath = path2.join(util2.WINDIR, "System32", "DriverStore", "FileRepository");
          const candidates = fs.readdirSync(basePath, { withFileTypes: true }).filter((dir) => dir.isDirectory()).map((dir) => {
            const nvidiaSmiPath = path2.join(basePath, dir.name, "nvidia-smi.exe");
            try {
...
function requireFilesystem() {
  ...
  const exec = require$$1.exec;
  const execSync = require$$1.execSync;
  const execPromiseSave = util2.promisifySave(require$$1.exec);
  const _platform = process.platform;
  const _linux = _platform === "linux" || _platform === "android";
  const _darwin = _platform === "darwin";
  const _windows = _platform === "win32";
...
function requireBluetooth() {
  ...
  const path2 = require$$2;
  const util2 = requireUtil();
  const bluetoothVendors2 = requireBluetoothVendors();
  const fs = require$$1$1;
  const _platform = process.platform;
  const _linux = _platform === "linux" || _platform === "android";
  const _darwin = _platform === "darwin";
  ...
  function parseBluetoothVendor(str) {
    ...
    if (!isNaN(id)) return bluetoothVendors2[id];
  }
...
function requireLib() {
  ...
  (function(exports$1) {
    ...
    const util2 = requireUtil();
    const system2 = requireSystem();
    const osInfo = requireOsinfo();
    const cpu2 = requireCpu();
    const memory2 = requireMemory();
    const battery2 = requireBattery();
    const graphics2 = requireGraphics();
    const filesystem2 = requireFilesystem();
    const network2 = requireNetwork();
    const wifi2 = requireWifi();
    const processes2 = requireProcesses();
    const users2 = requireUsers();
    const internet2 = requireInternet();
    const docker2 = requireDocker();
    const vbox = requireVirtualbox();
    const printer2 = requirePrinter();
    const usb2 = requireUsb();
    const audio2 = requireAudio();
    const bluetooth2 = requireBluetooth();
    const _platform = process.platform;
    const _windows = _platform === "win32";
    const _freebsd = _platform === "freebsd";
    const _openbsd = _platform === "openbsd";
      ...
      return new Promise((resolve) => {
        process.nextTick(() => {
          ...
          Promise.all([
            ...
            cpu2.cpuFlags(),
            graphics2.graphics(),
            network2.networkInterfaces(),
            memory2.memLayout(),
            filesystem2.diskLayout(),
            audio2.audio(),
            bluetooth2.bluetoothDevices(),
            usb2.usb(),
            printer2.printer()
          ]).then((res) => {
            data.system = res[0];
    ...
    function getDynamicData(srv, iface, callback) {
      ...
      return new Promise((resolve) => {
        process.nextTick(() => {
          iface = iface || network2.getDefaultNetworkInterface();
          srv = srv || "";
          ...
          if (!_openbsd && !_freebsd && !_netbsd && !_sunos) {
            network2.networkStats(iface).then((res) => {
              data.networkStats = res;
              functionProcessed();
            });
          }
          if (!_sunos) {
            network2.networkConnections().then((res) => {
              data.networkConnections = res;
              functionProcessed();
            });
          }
          memory2.mem().then((res) => {
            data.mem = res;
            functionProcessed();
          });
          if (!_sunos) {
          ...
          if (!_sunos) {
            filesystem2.fsSize().then((res) => {
              data.fsSize = res;
              functionProcessed();
            });
          }
          if (!_windows && !_openbsd && !_freebsd && !_netbsd && !_sunos) {
            filesystem2.fsStats().then((res) => {
              data.fsStats = res;
              functionProcessed();
            });
          }
          if (!_windows && !_openbsd && !_freebsd && !_netbsd && !_sunos) {
            filesystem2.disksIO().then((res) => {
              data.disksIO = res;
              functionProcessed();
            });
          }
          if (!_openbsd && !_freebsd && !_netbsd && !_sunos) {
            wifi2.wifiNetworks().then((res) => {
              data.wifiNetworks = res;
              functionProcessed();
            });
          }
          internet2.inetLatency().then((res) => {
    ...
    exports$1.mem = memory2.mem;
    exports$1.memLayout = memory2.memLayout;
    exports$1.battery = battery2;
    exports$1.graphics = graphics2.graphics;
    exports$1.fsSize = filesystem2.fsSize;
    exports$1.fsOpenFiles = filesystem2.fsOpenFiles;
    exports$1.blockDevices = filesystem2.blockDevices;
    exports$1.fsStats = filesystem2.fsStats;
    exports$1.disksIO = filesystem2.disksIO;
    exports$1.diskLayout = filesystem2.diskLayout;
    exports$1.networkInterfaceDefault = network2.networkInterfaceDefault;
    exports$1.networkGatewayDefault = network2.networkGatewayDefault;
    exports$1.networkInterfaces = network2.networkInterfaces;
    exports$1.networkStats = network2.networkStats;
    exports$1.networkConnections = network2.networkConnections;
    exports$1.wifiNetworks = wifi2.wifiNetworks;
    exports$1.wifiInterfaces = wifi2.wifiInterfaces;
    exports$1.wifiConnections = wifi2.wifiConnections;
    ...
    exports$1.inetChecksite = internet2.inetChecksite;
    exports$1.inetLatency = internet2.inetLatency;
    exports$1.dockerInfo = docker2.dockerInfo;
    exports$1.dockerImages = docker2.dockerImages;
    exports$1.dockerContainers = docker2.dockerContainers;
    exports$1.dockerContainerStats = docker2.dockerContainerStats;
    exports$1.dockerContainerProcesses = docker2.dockerContainerProcesses;
    exports$1.dockerVolumes = docker2.dockerVolumes;
    exports$1.dockerAll = docker2.dockerAll;
    exports$1.vboxInfo = vbox.vboxInfo;
    exports$1.printer = printer2.printer;
    exports$1.usb = usb2.usb;
    exports$1.audio = audio2.audio;
    exports$1.bluetoothDevices = bluetooth2.bluetoothDevices;
...
const si = /* @__PURE__ */ getDefaultExportFromCjs(libExports);
class SysInfoInfra {
  async getGPULoad() {
...
class SysInfoEventEmitter extends EventEmitter {
  running = false;
  interval = 1e3;
  sysInfoService;
  constructor() {
  ...
  async start() {
    ...
    while (this.running) {
      try {
        const snapshot = await this.sysInfoService.getAll();
...
class SysInfoModule extends BaseModule {
  prefix = "sysinfo";
  controller;
  constructor() {
    super();
  ...
  getHandlers() {
    const handlers = {};
    Object.getOwnPropertyNames(SysInfoController.prototype).filter((name) => name !== "constructor").forEach((name) => {
      handlers[name] = this.controller[name].bind(this.controller);
    });
    return handlers;
  }
  setupEvents(win2) {
    ...
    emitter.on("updated", (data) => {
      win2.webContents.send(`${this.prefix}:updated`, data);
    });
